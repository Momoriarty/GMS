...
class HomeController extends Controller
{
    ...
    {
        $data = Cache::remember('home_stats', 60, function () {
            return [
                'teams' => Tim::count(),
    ...
    {
        $matches = Cache::remember('home_upcoming_matches', 60, function () {
            return JadwalPertandingan::with(['tim1', 'tim2', 'event'])
                ->where('status', 'terjadwal')
                ->where('waktu_pertandingan', '>', now())
    ...
    public function events()
    {
        $events = Cache::remember('home_events', 60, function () {
            return Event::withCount(['pendaftaran' => function ($query) {
