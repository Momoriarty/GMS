...
function PaymentResult({ paymentData, paymentMethod, event, namaTim, paymentStatus, onClose }) {
  ...
  const amountToPay = paymentData?.gross_amount || event?.biaya_pendaftaran;
  const total = fmtCurrency(amountToPay);
      ...
      <div className={`${shell} border-white/10 text-center`}>
        ...
        <div className="bg-white rounded-xl p-4 inline-block mx-auto">
          {(() => {
            ...
            return qrUrl ? (
              <img src={qrUrl} alt="QRIS" className="w-48 h-48 object-contain mx-auto" />
            ) : (
...
export default function EventDetail() {
    ...
    const setupEcho = async () => {
      try {
        ...
        window.Echo = new Echo({
          broadcaster: "pusher",
          key: import.meta.env.VITE_PUSHER_APP_KEY || import.meta.env.VITE_PUSHER_KEY || "app-key",
  ...
  const handleToggleForm = () => {
    ...
    api
      .get("/tim/my-teams", { params: { event_id: id } })
      .then((res) => {
